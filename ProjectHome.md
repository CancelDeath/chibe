...
Note: ChiBE cannot run on Mac OS X with Java 1.7. This is because Apple decided to remove 32-bit support from Java 1.7. We recommend using Java 1.6 with Mac OS X. [Here](MacAndJava6.md) is how to do it.
...
Here is the [latest build for Mac](http://cbio.mskcc.org/~ozgun/chibe-latest-build-macosx-x86.zip), [latest build for Windows](http://cbio.mskcc.org/~ozgun/chibe-latest-build-windows.zip), and [latest build for Linux](http://cbio.mskcc.org/~ozgun/chibe-latest-build-linux.tar.gz) (beware: this is not a release).

Please see the SetupGuide for running ChiBE from sources, and QuickStartGuide for fast examples.

ChiBE 2.2 User's Guide can be reached [here](http://www.cs.bilkent.edu.tr/~ivis/chibe/ChiBE-2.2.UG.pdf).

<a href='Hidden comment: 
ChiBE 2.1 User"s Guide can be reached [http://www.cs.bilkent.edu.tr/~ivis/chibe/ChiBE-2.1.UG.pdf here].
'></a>
